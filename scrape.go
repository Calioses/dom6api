...
/*
Table 'items' has columns: [id name type constlevel mainlevel mpath gemcost]
Table 'spells' has columns: [id name gemcost mpath type school researchlevel]
Table 'units' has columns: [id name hp size mount co_rider]
Table 'sites' has columns: [id name path level rarity]
Table 'mercs' has columns: [id name bossname commander_id unit_id nrunits]
*/
//TODO route correctly to the slightly altered file structure.
//TODO change db handler.
//TODO make sure that I have the scraping correct
...
func Scrape() {
	...
	types := []string{"item", "spell", "unit", "site", "merc"}
	for _, typ := range types {
		...
		for i := 0; i < n; i++ {
			...
			locator.Screenshot(playwright.LocatorScreenshotOptions{
...
// TODO add mount and corider to the database
// -------------------- UNITS --------------------
